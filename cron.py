...
from id_model import Classroom, Program, Reminder
from named_model import Aggregator, ErrorChecker, Indexer
import config
import mandrill
import util
...
""".format(''.join([self._reminder_summary(r) for r in reminders]))
        ...
        mandrill.send(
            ...
            subject="{} Reminder Emails sent today".format(
                len(reminders)),
            body=mandrill.render_markdown(body),
        )

    def _reminder_summary(self, reminder):
        """Render the details of an email in way good for digests."""
        return ("\n\n"
                "----\n\n"
                "**Send date**: {}\n\n"
                "**To**: {}\n\n"
                "**Subject**: {}\n\n"
                "{}".format(reminder['send_date'], reminder['to_address'],
