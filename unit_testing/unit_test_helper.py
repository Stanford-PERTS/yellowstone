...
class PertsTestCase(unittest.TestCase):
    ...
    def populate(self):
        ...
        self.school_admin = self.internal_api.create(
            'user', {'user_type': 'school_admin', 'program': self.program.id})
        ...
        self.teacher = self.public_api.create(
            'user', {'user_type': 'teacher', 'program': self.program.id})
        self.teacher.put()
