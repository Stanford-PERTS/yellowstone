...
PertsApp.controller('ClientTestController', [
    '$scope', 'testProgramId', '$pertsAjax', '$q', '$emptyPromise', '$timeout', '$window',
    function ($scope, testProgramId, $pertsAjax, $q, $emptyPromise, $timeout, $window) {
        'use strict';
        ...
        // To make tests fail, either use util.assert(), throw an Error, or return
        // a promise that gets rejected.
        $scope.exampleTests = {
            syncTest: function () {
                util.assert(true === true, "This test succeeds.");
            },
            syncFailedTest: function () {
                util.assert(true === false, "This test fails.");
            },
            asyncTest: function () {
                var d = $q.defer();
        ...
        $scope.tests = {
            browserTest: function () {
                ...
                util.assert(!html.hasClass('ie6') && !html.hasClass('ie7'),
                       oldBrowserMsg);
            },
            qualtricsTest: function () {
                ...
                var returnPromise = $pertsAjax({
                    ...
                    var url = 'https://sshs.qualtrics.com/SE/?SID=SV_6PeouYAZMvCIUAd' +
