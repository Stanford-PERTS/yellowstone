...
util.preventBackspaceNavigation();
...
var PertsApp = angular.module(
    ...
    ['$interpolateProvider', '$tooltipProvider', function ($interpolateProvider, $tooltipProvider) {
    'use strict';
...
PertsApp.factory('$pertsAjax', function ($http, $q, $window) {
    ...
    return function $pertsAjax(unsafeKwargs) {
        ...
        forEach(kwargs.params, function (k, v) {
            ...
            if (v instanceof Array && kwargs.method === 'GET') {
                // Sometime after 1.0.7, angular stopped automatically
                // JSONifying arrays. But we like that feature, so put it back.
                kwargs.params[k] = JSON.stringify(v);
            }
        });
...
function entityStoreFactory($q, $emptyPromise, $pertsAjax, $timeout, verb) {
    ...
    // Is the given property key a private property?
    var isPrivate = function (k) {
        // The underscore prefix is a PERTS convention.
        // The double dollar prefix is an angular convention.
        return k.substring(0, 1) === '_' || k.substring(0, 2) === '$$';
    };

    var isPublic = function (k) {
        return !isPrivate(k);
    };
    ...
    return {
        ...
        set: function (newEntity) {
            ...
            var changedKeys = util.dictionaryDiff(newEntity, this._m[id]);
            if (changedKeys.length > 0) {
                forEach(changedKeys, function (k) {
                    this._m[id][k] = newEntity[k];
                }, this);
                this._setCache(id, newEntity);
            }
            return this._m[id];
        },
        ...
        _setCache: function (id, newEntity) {
            forEach(newEntity, function (k, v) {
                if (!isPrivate(k)) {
                    // copying important for arrays and objects
                    this._cache[id][k] = angular.copy(v);
                }
            }, this);
        },
        load: function (id_or_ids) {
            ...
            if (typeof id_or_ids === 'string') {
                ...
                ids = util.arrayUnique(id_or_ids);
            }
            var missingIds = [];
        ...
        synchronize: function () {
            ...
            forEach(self._m, function (id, entity) {
                // get changes
                var keysToUpdate = util.dictionaryDiff(entity, self._cache[id])
                    .filter(isPublic);
                if (keysToUpdate.length > 0) {
                    var params = {};
                    forEach(keysToUpdate, function (k) {
                        var v = entity[k];
                        // Some values ("lists" and "dictionaries") need to be
                        // JSON-encoded. The '_json' suffix on the parameter
                        // signals the server to accept this encoding.
                        var jsonEncoding = angular.isArray(v) || util.isDictionary(v);
                        k = jsonEncoding ? k + '_json' : k;
                        params[k] = v;
                    });
                    ...
                    var p = $pertsAjax({
                        ...
                        // cache.
                        self._setCache(response.id, response);
        ...
        watchForUpdates: function () {
            if (this._failedPuts < 10) {
                ...
                util.displayUserMessage('error', "Connection lost, please reload.");
            }
        }
    };
}
    ...
    return {
        ...
        link: function (scope, element, attrs) {
            element.bind('blur', function () {
                ...
                element.addClass('perts-blurred')
            });
        }
    };
});
...
PertsApp.directive('chosenSelect', function () {
    ...
    return function postLink(scope, element, attr) {
        ...
        // Copy these attributes in the html over to the chosen widget.
        ...
        forEach(availableOptions, function (underscoreOption) {
            var camelOption = util.separatedToCamel(underscoreOption, '_'),
                optionValue = scope.$eval(attr[camelOption]);
            if (optionValue !== undefined) {
                buildOptions[underscoreOption] = optionValue;
            }
        });

        // Activate the chosen plugin on this select element (it's already
        // been wrapped by jQuery!).
        element.chosen(buildOptions);

        // Link the focus and blur events of the <select> to the text input
        // created by the chosen widget.
        $('input', element.next()).focus(function () {
            scope.$apply(attr.ngFocus);
        }).blur(function () {
            scope.$apply(attr.ngBlur);
            element.next().addClass('perts-blurred')
        });
    };
});
...
// Takes out anything but alphabetic, spaces, hyphens, and apostrophes.
PertsApp.directive('nameCharactersOnly', function () {
    'use strict';
    return {
        require: 'ngModel',
        link: function (scope, element, attrs, modelCtrl) {
...
PertsApp.directive('dropdown', function () {
    'use strict';
    return {link: function (scope, element) {
        element.dropdown();
    }};
...
PertsApp.directive('intlTelInput', function () {
    'use strict';
    return {link: function (scope, element) {
        ...
        element.bind('change', function (event) {
            ...
            // Wait until angular is done add/removing classes, then...
            setTimeout(function () {
                if (isValid) {
...
PertsApp.filter('orderObjectBy', function() {
